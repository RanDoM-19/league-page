...
<div class="constitution">
    ...
    <h3 class="noUnderscore clickable" on:click={() => goToSection(one)}>Section 1: Roster</h3>
        ...
        <h4 class="noUnderscore clickable" on:click={() => goToSection(oneTwo)}>1.2 Trading</h4>
           ...
           <!--<h5 class="noUnderscore clickable" on:click={() => goToSection(oneTwoThree)}>1.2.3 Trade Deadline</h5>-->
            <h5 class="noUnderscore clickable" on:click={() => goToSection(oneTwoFour)}>1.2.2 Trade Restrictions</h5>
    ...
    <p>
        Trading of players and draft picks (up to 3 drafts away) are allowed.
        All trades made before 12:00 am on Sunday will be put through before games start that day.
        Trades involving players who have already had their game for the week will be processed on Tuesday (Pending no game Tuesday game).
    </p>
    
    <h4 bind:this={oneTwoOne}>1.2.1 Trade Collusion</h4>
    <div class="subBlock">
        <p>All trades can be investigated by the commissioner to ensure that no outside factors were involved in the trade, Commissioner has full authority to override a trade that involved outside factors or that broke league integrity 
           All trades will be looked at for collusion or the possibility of breaking rules, league members can bring any information that contains possible evidence of collusion or other rule breaking to the commissioner for further review.
           If trade talks are done over a text chat system the commissioner can request screen shots of those discussions to use as evidence regarding possible collusion.</p>
  ...
  <!--  <h4 bind:this={oneTwoTwo}>1.2.2 Lending Players Prohibition</h4>
   ...
   <h4 bind:this={oneTwoTwo}>1.2.2 Trade Restrictions</h4>
    <div class="subBlock">
      <p>You will not be able to trade draft picks that are more than 3 drafts away. For example, during the 2022 season you cannot trade any picks in or after the 2025 draft. You may only trade FAAB $ from the current season.</p>
    </div>
    ...
    <p>The draft order each year will be a linear draft in reverse order of the previous year’s standings.</p>

    <h3 bind:this={twoTwo}>2.2 Rookie Drafts</h3>
    <p>Rokkie drafts will consist of 5 round of drafting, with 8 hours alotted for each pick. The draft wil take place after the NFL Draft and before Aug 1 each year.</p>
    ...
    <p>The league scoring system breakdown is listed on the Sleeper fantasy app (for the most part, it follows typical .5PPR Scoring 1 PPR TE). The scoring system will never change without a majority 70% league vote.</p>
